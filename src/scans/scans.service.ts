...
export class ScansService {
  ...
  async getScansByPatientUser(userId: string): Promise<any[]> {
    ...
    return scans.map(scan => ({
      ...
      result: scan.result,
      confidence: scan.confidence ? (scan.confidence <= 1.0 ? Math.round(scan.confidence * 10000) / 100 : scan.confidence) : null,
      status: scan.status,
  ...
  async updateScan(
    ...
    return {
      ...
      confidence: updatedScan.confidence ? (updatedScan.confidence <= 1.0 ? Math.round(updatedScan.confidence * 10000) / 100 : updatedScan.confidence) : null,
